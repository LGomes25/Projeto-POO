...
public class FolhaPagamento extends Funcionario implements CalcSalInterface {
	...
	private Double calcularINSS() {
		if (getSalarioBruto() <= 1518.0) {
			Double faixa1 = Math.round(((getSalarioBruto() * 0.075) - 0.) * 100.0) / 100.0;
			return faixa1;
		} else if (getSalarioBruto() >= 1518.01 & getSalarioBruto() <= 2793.88) {
			Double faixa2 = Math.round(((getSalarioBruto() * 0.09) - 22.77) * 100.0) / 100.0;
			return faixa2;
		} else if (getSalarioBruto() >= 2793.89 & getSalarioBruto() <= 4190.83) {
			Double faixa3 = Math.round(((getSalarioBruto() * 0.12) - 106.60) * 100.0) / 100.0;
			return faixa3;
		} else if (getSalarioBruto() >= 4190.84 & getSalarioBruto() <= 8157.41) {
			Double faixa4 = Math.round(((getSalarioBruto() * 0.14) - 190.42) * 100.0) / 100.0;
			return faixa4;
		} else {
	...
	private Double calcularIR() {
		...
		Double salBase = salarioBruto - descDep - 951.62;
		...
		if (salBase <= 2259.20) {
			...
			Double faixair1 = (salBase * 0.075) - 169.44;
			return faixair1;
		} else if(salBase <= 3751.05) {
			Double faixair2 = (salBase * 0.15) - 381.44;
			return faixair2;
		} else if(salBase <= 4664.68) {
			Double faixair3 = (salBase * 0.225) - 662.77;
			return faixair3;
		} else {
			Double faixair4 = (salBase * 0.275) - 896.00;
			return faixair4;
		}	
	}
